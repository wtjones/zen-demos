...
int main(int argc, const char** argv)
{
    ...
    RasPosixSettings plat_settings = { 0 };
    RasResult result = posix_script_load_settings("settings/app.lsp", &plat_settings);

    if (result == RAS_RESULT_ERROR) {
    ...
    SDL_Window* win = SDL_CreateWindow(
        ...
        100,
        100,
        plat_settings.screen_settings.screen_width * plat_settings.screen_settings.scale,
        plat_settings.screen_settings.screen_height * plat_settings.screen_settings.scale,
        SDL_WINDOW_SHOWN);
    ...
    surface = SDL_CreateRGBSurface(
        0,
        plat_settings.screen_settings.screen_width,
        plat_settings.screen_settings.screen_height,
        8, 0, 0, 0, 0);
    if (!surface) {
    ...
    result = ras_app_init(argc, argv, &plat_settings.screen_settings);
    ...
    console = core_console_init(&plat_settings.screen_settings);
    if (console == NULL) {
    ...
    while (!should_quit) {
        ...
        if (states[RAS_LAYER_SCENE].max_frames == UINT32_MAX || states[RAS_LAYER_SCENE].current_frame < states[RAS_LAYER_SCENE].max_frames) {
            ...
            for (size_t i = 0; i < RAS_LAYER_COUNT; i++) {
                states[i].screen_settings.screen_width = plat_settings.screen_settings.screen_width;
                states[i].screen_settings.screen_height = plat_settings.screen_settings.screen_height;
            }
