...
RasResult core_renderdata_gridmaps_alloc(RasRenderData* render_data)
{
    for (size_t i = 0; i < render_data->scene->num_gridmaps; i++) {
        ...
        size_t max_verts = RAS_MESH_CLIP_PAD((gridmap->depth + 1) * (gridmap->width + 1) * 4);
        size_t max_faces = RAS_MESH_CLIP_PAD(gridmap->depth * gridmap->width * 12);
...
RasResult core_renderdata_tombmaps_alloc(RasRenderData* render_data)
{
    for (size_t i = 0; i < render_data->scene->num_tombmaps; i++) {
        ...
        for (size_t j = 0; j < tombmap->num_rooms; j++) {
            ...
            size_t max_verts = RAS_MESH_CLIP_PAD((room->num_sectors_x + 1) * (room->num_sectors_z + 1) * 4);
            size_t max_faces = RAS_MESH_CLIP_PAD(room->num_sectors_x * room->num_sectors_z * 6);
            if (core_pipeline_mesh_alloc(
                    max_verts,
                    max_faces * 3,
                    max_faces,
        ...
        *num_dest_materials = 0;
        ...
        for (uint32_t i = 0; i < element->num_indexes; i += 3) {
            ...
            if (face_clip_flags != 0 && clip_on) {
                ...
                assert(mesh->num_verts + num_out_verts <= mesh->max_verts);
                ...
                assert(*num_dest_faces + num_out_verts / 3 <= mesh->max_visible_faces);
                assert(*num_dest_materials + num_out_verts / 3 <= mesh->max_material_indexes);
