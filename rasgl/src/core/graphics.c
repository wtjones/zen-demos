...
void core_renderstate_init(RenderState* state)
{
    ...
    state->z_divide_mode = RAS_Z_DIVIDE_MODE_RT;
...
void core_renderstate_meshes_clear(RenderState* state)
{
    for (uint32_t i = 0; i < state->num_meshes; i++) {
        RasPipelineMesh* mesh = &state->meshes[i];
...
void core_renderstate_clear(RenderState* state)
{
    ...
    memset(state->visible_indexes, 0, sizeof(state->visible_indexes));
    memset(state->visible_faces, 0, sizeof(state->visible_faces));
    core_renderstate_meshes_clear(state);
}
    ...
    RasPipelineMesh* mesh)
{
    RasResult result = core_pipeline_mesh_alloc(
        RAS_MESH_CLIP_PAD(element->max_verts),
        RAS_MESH_CLIP_PAD(element->max_indexes),
        RAS_MESH_CLIP_PAD(element->max_faces),
        RAS_MESH_CLIP_PAD(element->max_material_indexes),
        mesh);
